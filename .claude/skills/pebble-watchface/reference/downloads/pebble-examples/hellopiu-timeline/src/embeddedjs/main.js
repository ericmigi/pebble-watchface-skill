...
const Gothic18 = Style.template({ font: "bold 18px Gothic", color: [BLACK, GRAY] });
...
const Header = Label.template($ => ({
	anchor: "HEADER", top: 0, height: 30, left: 0, right: 0, Skin: HeaderSkin,
...
class TransitionContainerBehavior extends Behavior {
	...
	easeOut(container) {
		let timeline = this.timeline = new Timeline();
		let animatedItem = container.first, bottom = container.y+container.height-(animatedItem.height + 2);
...
const EasingFunctionApplication = Application.template($ => ({
	Style: Gothic18, Skin: BackgroundSkin,
	contents: [
		Column($, {
			...
			contents: [
				new Header($),
				Container($, {
					anchor: "TRANSITION_CONTAINER", top: 15, bottom: 15, left: 15, right: 15, Skin: BorderedSkin,
					contents: [
						Content($, {
							bottom: 2, height: 25, width: 25, Skin: GraySkin
						}),
					],
...
const application = new EasingFunctionApplication({}, {});
